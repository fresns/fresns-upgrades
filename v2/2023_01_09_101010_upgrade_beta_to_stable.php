...
class UpgradeBetaToStable extends Migration
{
    ...
    {
        info('Migration: ', ['UpgradeBetaToStable']);

        $currentVersionInt = AppUtility::currentVersion()['versionInt'] ?? 100;

        if ($currentVersionInt == 100) {
            info('Migration: ', ['currentVersionInt == 100']);
            return;
        }
        ...
        if (! Schema::hasColumn('session_logs', 'app_id')) {
            Schema::table('session_logs', function (Blueprint $table) {
                $table->string('app_id', 8)->after('version')->nullable();
            });
        }
        ...
        if (! Schema::hasColumn('session_tokens', 'user_token')) {
            Schema::table('session_tokens', function (Blueprint $table) {
                ...
                $table->string('app_id', 8)->nullable()->after('version');
                $table->string('user_token', 32)->nullable()->after('account_token');
